...
return array(
     ...
     * 3. 配置信息 - 必须使用前缀 `settings.`，对应 `setting_config_path` 目录下的文件，如：默认配置下，
     *              `setting.site` 访问的是 `administrator/setting/site.php` 文件
     ...
     * @type string
     */
    // 设置后台主页视图文件，由 `user_dashboard` 选项决定
    'dashboard_view' => '',
     ...
     * @type string
     */
    // 用来作为后台主页的菜单条目，由 `user_dashboard` 选项决定，菜单指的是 `menu` 选项
    'home_page' => 'topics',
     ...
     * @type string
     */
    // 当选项 `permission` 权限检测不通过，会重定向用户到此处设置的路径
